...
| 能力 | 说明 |
| --- | --- |
| 开屏入场动画（K95 式） | 纯白背景，照片卡从屏幕正中央**逐张"绽放"长大**（宽度 0→目标、强 ease-out、纵横比锁定），目标宽度 84%→100% 逐张递增、一张压一张，最后一张 100% 的**纯黑卡**盖顶；右上角 `[0..100]` 计数器与发牌同钟。随后黑卡（同位黑色补丁）以 expo.inOut 用 1s **从中心扩张铺满全屏**——黑色盖满后才揭示：黑幕淡出的同时，墙上的照片**在各自位置一张一张弹出**（55ms 错峰 + easeOutBack 微回弹） |
| 曲面照片（柱面抛物面） | 每张照片是一张按 `z = −a·x²` 弯曲的抛物柱面薄片，贴合圆柱墙的弧度；双面显示，背面在着色器内翻转 U（`gl_FrontFacing`）保证正像；无边框盒体、无任何深色几何 |
| 悬停整体缩放 | 鼠标悬停照片：整张卡片统一阻尼放大至 1.08，移开平滑回落；同时自转减速（三档模型） |
| 速度弯曲（一体式） | 滚轮滚动时**整张卡片（照片与轮廓一体）几何弯曲**——纯顶点位移，无任何 UV/纹理层形变，画面内容与卡片刚性锁定；强度随带符号的滚动速度，停止后阻尼归零 |
| 三档动态自转 | 正常 0.09 · 悬停照片减速 0.03 · 滚动中提速 0.20（rad/s，约 2 倍常速），滚动档方向跟随滚动方向（向上滚则反转），档位间用阻尼平滑混合，无生硬跳变 |
...
| --- | --- | --- | --- |
| `SPIN.BASE` | 默认陈列 | `0.09` | ≈ 70s |
| `SPIN.HOVER` | 指针悬停在任意照片上 | `0.03` | ≈ 209s |
| `SPIN.SCROLL` | 滚轮滚动中（约 2 倍常速；**方向随滚动方向**，向上滚为 −0.20 反转） | `0.20` | ≈ 31s |
| `SPIN.DAMP` | 档位切换阻尼（越大切换越干脆，时间常数 ≈ 1/DAMP 秒） | `6` | — |
| `SPIN.SCROLL_HOLD` | 最后一次滚轮输入后仍算"滚动中"的时长 | `450ms` | — |
...
└── src/
    ...
    ├── scene/
    │   ├── PhotoWall.js        # 圆柱阵列曲面照片 + 悬停/入场/浮动状态机
    │   ├── photoMaterial.js    # 照片着色器：双面正像 + 整体几何弯曲（无 UV 形变）
...
| `WALL.ROW_GAP` | 层距（越大垂直间隔越宽；当前层间净空约 1.8 倍卡高） | `9.0` |
| `WHEEL.TRAVEL` | 滚轮全程垂直行程（= 2 × 层距） | `18` |
