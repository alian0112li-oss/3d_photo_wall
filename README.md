...
基于 **Three.js + Vite** 的全屏 3D 照片墙。照片以真正的 3D 立体卡片沿圆柱面环绕排布 —— 正面照片、背面品牌背板、金属质感边框，任何角度观看都完整。整套交互建立在**磁性跟随 + 阻尼缓动插值**的运动系统上：所有输入只更新目标值，帧循环用帧率无关的指数阻尼追赶目标，产生黏性、粘滞、有重量的丝滑手感。
...
└── src/
    ...
    ├── scene/
    │   ├── PhotoWall.js        # 圆柱阵列 3D 卡片 + 磁性悬停 + 浮动
