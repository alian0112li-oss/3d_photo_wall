...
| 能力 | 说明 |
| --- | --- |
| 开屏入场动画 | 白底发牌：照片从视口顶端**可见地**落入屏幕正中央（全程不透明），正位同点堆叠、一张压一张；每张落下时照片在固定框内快速微放大（1 → 1.1）；最后落下唯一的黑卡，经 **FLIP**（First → Last → Invert → Play）转化为全屏"视口遮罩层"——黑色从屏幕中央的卡片位置**向四周扩散**盖满视口，同步切换底层排版，再淡出揭示 3D 墙 |
