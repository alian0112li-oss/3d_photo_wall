...
| 能力 | 说明 |
| --- | --- |
| 开屏入场动画 | 白底发牌：照片逐张淡入下落到屏幕正中央、正位同点堆叠、一张压一张，最后落下唯一的黑卡；随后黑卡经 **FLIP**（First → Last → Invert → Play）瞬间转化为全屏"视口遮罩层"——页面由白转黑，盖屏同时同步切换底层排版，再淡出揭示 3D 墙 |
| 双面照片卡片 | BoxGeometry 厚度边框，正反两面都是照片正像（旋转非镜像），没有"背面" |
| 三档动态自转 | 正常 0.14 · 悬停照片减速 0.05 · 滚动中微加速 0.19（rad/s），档位间用阻尼平滑混合，无生硬跳变 |
...
| `SPIN.SCROLL` | 滚轮滚动中（比正常快一点点） | `0.19` | ≈ 33s |
